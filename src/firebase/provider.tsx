...
import { Auth, User, onAuthStateChanged, type IdTokenResult } from 'firebase/auth';
...
export const FirebaseProvider: React.FC<FirebaseProviderProps> = ({
  ...
  useEffect(() => {
    ...
    setUserAuthState({ user: null, isUserLoading: true, userError: null, claims: null });
      ...
      async (firebaseUser) => {
        if (firebaseUser) {
          try {
            ...
            setUserAuthState({ user: firebaseUser, isUserLoading: false, userError: null, claims: idTokenResult.claims });
          } catch (error) {
             console.error("FirebaseProvider: getIdTokenResult error:", error);
             setUserAuthState({ user: firebaseUser, isUserLoading: false, userError: error as Error, claims: null });
          }
        } else {
...
export const useUser = (): UserHookResult => {
  const { user, isUserLoading, userError, claims } = useFirebase();
  return { user, isUserLoading, userError, claims };
};
